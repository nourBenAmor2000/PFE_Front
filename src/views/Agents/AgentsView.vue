    ...
    <div class="p-6 space-y-6">
        ...
        <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
          <div>
            <nav class="text-sm text-gray-500 mb-1">
              ...
              <span class="text-orange-600 font-medium">Agents</span>
            </nav>
            <div class="flex items-center gap-3">
              ...
              <div>
                <h1 class="text-2xl md:text-3xl font-extrabold tracking-tight text-gray-900">Gestion des Agents</h1>
                <p class="text-gray-600">Gérez les agents de votre organisation</p>
              </div>
            </div>
          </div>
          ...
          <div class="flex items-center gap-2">
            ...
            <Button class="inline-flex items-center gap-2 rounded-lg bg-orange-600 text-white px-4 py-2 font-medium hover:bg-orange-700" @click="goToAdd">
              <Plus class="w-4 h-4" />
              Ajouter un agent
            </Button>
          </div>
        ...
        <Card class="rounded-xl border shadow-sm">
          <CardContent class="p-5">
            <div class="flex items-center gap-3">
              ...
              <div class="min-w-0">
                <p class="text-xs font-medium text-gray-500 uppercase tracking-wide">Agences</p>
                <p class="text-2xl font-bold text-gray-900">{{ uniqueAgencies }}</p>
              </div>
            </div>
          </CardContent>
        </Card>
      ...
      <section class="bg-white rounded-2xl border shadow-sm overflow-hidden">
        ...
        <div class="p-4 border-b flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
          <h2 class="text-lg font-semibold text-gray-900">Liste des agents</h2>
          ...
          <div class="flex flex-col sm:flex-row gap-3">
            ...
            <div class="flex items-center rounded-lg border border-gray-200 bg-white px-3 py-2">
              ...
              <input
                ...
                @input="onQuery"
                type="text"
                placeholder="Rechercher (nom, email, téléphone, agence)…"
                class="ml-2 bg-transparent text-sm outline-none placeholder:text-gray-400 w-64"
              />
            </div>

            <!-- Filters -->
            <select v-model="agency" @change="applyFilters" class="rounded-lg border-gray-200 text-sm">
              <option value="">Toutes les agences</option>
              <option v-for="ag in agencyList" :key="ag" :value="ag">{{ ag }}</option>
            </select>
          ...
          <Table>
            <TableHeader class="bg-gray-50">
              <TableRow>
                ...
                <TableHead class="font-semibold text-gray-900">Agence</TableHead>
              ...
              >
                ...
                <TableCell>
                  <div class="space-y-1">
                    <div class="flex items-center gap-2">
                      ...
                      <a :href="`mailto:${a.email}`" class="text-primary hover:underline truncate">{{ a.email || '—' }}</a>
                    </div>
                    <div class="flex items-center gap-2">
                      <Phone class="w-4 h-4 text-gray-400" />
                      <a :href="`tel:${a.phone}`" class="text-gray-700 truncate">{{ a.phone || '—' }}</a>
                    </div>
                  </div>
                </TableCell>
                ...
                <TableCell>
                  <div class="flex justify-end gap-2">
                    <Button variant="outline" size="sm" class="flex items-center gap-2" @click="goToEdit(a._id)">
                      <Edit class="w-4 h-4" />
                      Modifier
                    </Button>
                    <Button variant="destructive" size="sm" class="flex items-center gap-2" @click="confirmDelete(a)">
                ...
                <TableCell colspan="6">
                  <div class="text-center py-12">
                    ...
                    <p class="text-gray-500 mb-6">
                      {{ q || agency || position ? 'Essayez de modifier vos critères de recherche.' : 'Commencez par ajouter votre premier agent.' }}
                    </p>
                    <Button class="inline-flex items-center gap-2 rounded-lg bg-orange-600 text-white px-4 py-2 font-medium hover:bg-orange-700" @click="goToAdd">
                      <Plus class="w-4 h-4" />
                      Ajouter un agent
        ...
        <div class="border-t px-4 py-3 flex items-center justify-between text-sm">
          ...
          <div class="flex items-center gap-2">
            <button class="rounded-lg border px-3 py-1.5 hover:bg-gray-50 disabled:opacity-50" :disabled="page===1" @click="page--">Précédent</button>
            <button class="rounded-lg border px-3 py-1.5 hover:bg-gray-50 disabled:opacity-50" :disabled="page>=pages" @click="page++">Suivant</button>
          </div>
        </div>
      </section>
    </div>
...
const uniqueAgencies = computed(() => new Set(agents.value.map(a => a.agency_name).filter(Boolean)).size)
const agencyList = computed(() => [...new Set(agents.value.map(a => a.agency_name).filter(Boolean))].sort())
const positionList = computed(() => [...new Set(agents.value.map(a => a.position).filter(Boolean))].sort())

/* filtering */
const filtered = computed(() => {
  const s = q.value.toLowerCase()
  return agents.value
    .filter(a => (agency.value ? a.agency_name === agency.value : true))
...
watch([q, agency, position, perPage], () => { page.value = 1 })
...
const handleDelete = async () => {
  ...
  try {
    ...
    // rafraîchir la page courante si elle devient vide
...
const bulkDeactivate = async () => {
  // Nécessite une méthode updateAgent côté store
