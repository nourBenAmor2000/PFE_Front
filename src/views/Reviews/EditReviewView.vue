...
onMounted(async () => {
  try {
    ...
    const review = reviewStore.reviews.find(r => r._id === route.params.id || r.id === route.params.id)
    if (review) {
      client_id.value = review.client_id || ''
      logement_id.value = review.logement_id || ''
...
const submit = async () => {
  ...
  try {
    await reviewStore.updateReview(route.params.id, {
    ...
    const errorMsg = reviewStore.error || error.message || 'Erreur lors de la mise à jour'
