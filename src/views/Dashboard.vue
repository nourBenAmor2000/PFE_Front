<template>
  <div class="min-h-screen bg-gray-50">
      ...
      <div class="container mx-auto px-4 py-6 flex items-center justify-between">
        <div>
          <h1 class="text-2xl font-bold text-gray-900">
            Bonjour,
            <span class="text-[var(--brand-700)]">{{ user?.name }}</span> 👋
          </h1>
      ...
      <section class="mt-8 rounded-xl border bg-white shadow-sm">
        ...
        <div class="p-6">
          ...
          <section v-if="activeTab === 'search'">
            ...
            <!-- Filtres -->
            <div
              class="mb-6 grid grid-cols-1 gap-4 rounded-lg bg-gray-50 p-4 md:grid-cols-4"
            >
              ...
              <div class="flex items-end">
                <button @click="searchProperties" class="btn-primary w-full">
                  Rechercher
                </button>
              </div>
            </div>
            ...
            <div v-if="isLoadingProperties" class="flex justify-center py-10">
              <div
                class="h-8 w-8 rounded-full border-2 border-[var(--brand)] border-t-transparent animate-spin"
              ></div>
            </div>

            <template v-else>
              <EmptyState
                v-if="!availableProperties.length && !propertiesError"
                title="Aucun logement trouvé"
                caption="Essayez d’élargir votre zone de recherche ou d’assouplir les filtres."
              />
          ...
          <section v-else>
            <div class="mb-6 flex items-center justify-between">
              ...
              <button class="btn-primary" @click="showAddReviewModal = true">
                Laisser un avis
              </button>
            </div>
    ...
    >
        ...
        <form @submit.prevent="submitVisitRequest" class="space-y-4">
          ...
          <div class="grid grid-cols-1 gap-4 md:grid-cols-2">
            <FormField label="Date souhaitée">
              <input
                v-model="visitRequest.preferredDate"
                required
                type="date"
                class="input"
              />
            ...
            <FormField label="Heure souhaitée">
              <input
                v-model="visitRequest.preferredTime"
                required
                type="time"
                class="input"
              />
      ...
      <template #footer>
        <button class="btn-ghost" @click="showVisitModal = false">
          Annuler
        </button>
        <button class="btn-primary" @click="submitVisitRequest">
          Envoyer la demande
        </button>
      </template>
    </Modal>
    ...
    >
      ...
      <template #footer>
        <button class="btn-ghost" @click="showAddReviewModal = false">
          Annuler
        </button>
...
const statCards = computed(() => [
  {
    ...
    value: clientStats.value.scheduledVisits,
    icon: 'calendar',
    tone: 'blue'
  },
  {
    label: 'Contrats actifs',
    value: clientStats.value.activeContracts,
    icon: 'doc',
    tone: 'green'
  },
  {
    label: 'Favoris',
    value: clientStats.value.favorites,
    icon: 'heart',
    tone: 'purple'
  },
  {
    label: 'Paiements en attente',
    value: clientStats.value.pendingPayments,
    icon: 'coin',
    tone: 'orange'
  }
])
...
const buildImageUrl = (raw) => {
  if (!raw) {
    return 'https://via.placeholder.com/400x260?text=Logement'
  }
...
const fetchProperties = async () => {
  ...
  try {
    ...
    const { data } = await api.get(endpoint, {
      params: {
        location: searchFilters.value.location || undefined,
        max_price: searchFilters.value.maxPrice || undefined,
        type: searchFilters.value.type || undefined
      }
    })

    const raw = data.logements || data.data || data

    availableProperties.value = (raw || []).map((item) => ({
      id: item.id || item._id,
      title: item.titre || item.title || 'Logement',
      address: item.adresse || item.address || '',
      price: item.prix || item.price || 0,
      rooms: item.nb_pieces || item.rooms || 1,
      image: buildImageUrl(
    ...
    propertiesError.value =
      "Impossible de charger les logements. Veuillez réessayer."
...
const resetSearch = () => {
  searchFilters.value = { location: '', maxPrice: '', type: '' }
  fetchProperties()
}
...
const myVisits = ref([
  {
    id: 1,
    propertyTitle: 'Appt 2P - Marais',
    propertyAddress: '15 Rue des Rosiers',
...
const submitVisitRequest = () => {
  myVisits.value.push({
    ...
    dateTime: new Date(
      `${visitRequest.value.preferredDate}T${visitRequest.value.preferredTime}`
    ),
...
const myContracts = ref([
  {
    ...
    propertyAddress: '8 Place de la Bastille',
    monthlyRent: 1200,
...
<script>
/* Petits composants locaux dans le même fichier pour simplicité */
  ...
  setup(props, { emit }) {
    return () =>
      h('div', { class: 'border-b' }, [
        h(
          'nav',
          { class: '-mb-px flex gap-6 px-6 overflow-x-auto' },
          props.items.map((t) =>
            h(
              'button',
              {
...
export const StatCard = defineComponent({
  ...
  setup(p) {
    ...
    const icons = {
      ...
      heart:
        'M4.318 6.318a4.5 4.5 0 000 6.364L12 20.364l7.682-7.682a4.5 4.5 0 00-6.364-6.364L12 7.636 10.682 6.318a4.5 4.5 0 00-6.364 0z',
      coin: 'M12 8c-1.657 0-3 .9-3 2s1.343 2 3 2 3 .9 3 2-1.343 2-3 2M12 8V7m0 1v8'
    }
    return () =>
      h(
        'div',
        { class: 'rounded-lg bg-white shadow p-6 flex items-center' },
        [
  ...
  setup(props, { emit }) {
    ...
    return () =>
      h(
        ...
        {
          class:
            'overflow-hidden rounded-xl border bg-white shadow-sm hover:shadow-md transition flex flex-col'
        },
        [
          h('img', {
          ...
          h('div', { class: 'p-4 flex-1 flex flex-col' }, [
            h(
              'h3',
              { class: 'mb-1 font-semibold text-gray-900 line-clamp-1' },
            ...
            h(
              ...
              [
                h(
                  'span',
                  { class: 'text-lg font-bold text-[var(--brand-700)]' },
                  `${price}/mois`
                ),
                h(
                  'span',
                  { class: 'text-sm text-gray-500' },
                  `${props.property.rooms} pièce(s)`
                )
              ]
            ),
            h('div', { class: 'mt-auto flex items-center justify-between' }, [
              h(
                'button',
                {
                  class: 'btn-success',
                  onClick: () => emit('visit', props.property)
                },
                'Demander une visite'
              ),
              h(
                'button',
                {
                  class: [
                    'p-1 rounded-full',
                    props.property.isFavorite ? 'text-red-500' : 'text-gray-400'
                  ],
                  onClick: () => emit('favorite', props.property)
                },
                [
                  h(
                    'svg',
...
export const VisitItem = defineComponent({
  ...
  setup(props, { emit }) {
    ...
    return () =>
      h('div', { class: 'rounded-lg bg-gray-50 p-4' }, [
        h('div', { class: 'flex items-start justify-between' }, [
          h('div', { class: 'flex-1' }, [
            h('div', { class: 'mb-2 flex items-center gap-3' }, [
              h('h3', { class: 'font-medium text-gray-900' }, props.visit.propertyTitle),
              h(
                'span',
                {
                  class: `px-2 py-1 text-xs rounded-full ${badge(
                    props.visit.status
                  )}`
                },
                props.visit.status
            ...
            h(
              ...
              [
                h('div', { class: 'flex items-center gap-2' }, [
                  ...
                  fmt.format(new Date(props.visit.dateTime))
                ]),
                h('div', { class: 'flex items-center gap-2' }, [
                  h(
                    'svg',
                  ...
                  `Agent : ${props.visit.agentName}`
                ])
              ]
            )
          ]),
          props.visit.status === 'Programmée'
            ? h(
                'button',
                {
                  class: 'text-sm text-red-600 hover:text-red-800',
                  onClick: () => emit('cancel', props.visit.id)
                },
                'Annuler'
              )
            : null
...
export const ContractItem = defineComponent({
  ...
  setup(props, { emit }) {
    ...
    return () =>
      h('div', { class: 'rounded-lg bg-gray-50 p-6' }, [
        h('div', { class: 'mb-4 flex items-start justify-between' }, [
          h('div', null, [
            h('h3', { class: 'mb-1 font-semibold text-gray-900' }, props.contract.propertyTitle),
            h(
              'p',
              { class: 'text-sm text-gray-600' },
              props.contract.propertyAddress
            )
          ]),
          h(
            'span',
            { class: `px-3 py-1 text-sm rounded-full ${badge(props.contract.status)}` },
            props.contract.status
          )
        ]),
        h('div', { class: 'mb-4 grid grid-cols-1 gap-4 md:grid-cols-3' }, [
          h('div', null, [
            h('p', { class: 'text-sm text-gray-600' }, 'Loyer mensuel'),
            h(
              'p',
              { class: 'font-semibold text-gray-900' },
              cur.format(props.contract.monthlyRent)
            )
          ]),
          h('div', null, [
            h('p', { class: 'text-sm text-gray-600' }, 'Début du contrat'),
            h('p', { class: 'font-semibold text-gray-900' }, d(props.contract.startDate))
          ]),
          h('div', null, [
            h('p', { class: 'text-sm text-gray-600' }, 'Fin du contrat'),
            h('p', { class: 'font-semibold text-gray-900' }, d(props.contract.endDate))
          ])
        ]),
        h('div', { class: 'flex items-center justify-between' }, [
          h('div', { class: 'text-sm text-gray-600' }, `Agent : ${props.contract.agentName}`),
          h('div', { class: 'flex gap-2' }, [
            h(
              'button',
              {
                class: 'text-sm text-[var(--brand-700)] hover:underline',
                onClick: () => emit('view', props.contract)
              },
              'Voir détails'
            ),
            props.contract.status === 'Actif'
              ? h(
                  'button',
                  { class: 'btn-success', onClick: () => emit('pay', props.contract) },
                  'Payer'
                )
              : null
          ])
    ...
    return () =>
      h('div', { class: 'rounded-lg bg-gray-50 p-4' }, [
        h('div', { class: 'mb-2 flex items-start justify-between' }, [
          h('div', null, [
            h('h3', { class: 'font-medium text-gray-900' }, props.review.targetName),
            h(
              'p',
              { class: 'text-sm text-gray-600' },
              props.review.targetType === 'agent' ? 'Agent' : 'Agence'
            )
          ]),
          h('div', { class: 'flex items-center' }, [
            h(
              'div',
              { class: 'mr-2 flex text-yellow-400' },
              Array.from({ length: 5 }, (_, i) =>
                h(
                  'svg',
                  {
                    class: 'h-4 w-4',
                    viewBox: '0 0 24 24',
                    fill: i < props.review.rating ? 'currentColor' : '#E5E7EB'
                  },
                  [
                    h('path', {
                      d:
            ...
            h(
              'span',
              { class: 'text-sm text-gray-600' },
              fmt.format(props.review.date)
            )
          ])
        ]),
        h('p', { class: 'text-gray-700' }, props.review.comment)
      ])
  }
})
    ...
    return () =>
      h(
        ...
        [
          ...
          p.caption
            ? h('p', { class: 'mt-1 text-gray-500' }, p.caption)
            : null
        ]
      )
  }
